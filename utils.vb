...
Module utils
    Public IFACE_NAME As String = "KMT"
    Public Const INIFILE As String = "KMT.ini"
    ...
    Public Sub ConsoleWriter()
        Dim ConsoleLine As New ConsoleBuffer
        Dim ConsoleQLength = ConsoleBufferQ.Count
        Dim Character As String = ""

        If Len(ConsoleLogLine) = 0 Then
            If ConsoleQLength > 0 Then
                ConsoleLine = ConsoleBufferQ.Dequeue
                ConsoleLogLine = ConsoleLine.Message
                Console.ForegroundColor = ConsoleLine.BG
                Console.CursorSize = 100
                Console.Write(vbCrLf)
            End If
            ConsoleCharacter = 1
        Else
            If ConsoleCharacter > Len(ConsoleLogLine) Then

                ConsoleLogLine = ""
            Else
                If ConsoleQLength < 15 Then
                    Character = Mid(ConsoleLogLine, ConsoleCharacter, 1)
                    ConsoleCharacter = ConsoleCharacter + 1
                    Console.Write(Character)
                Else
                    Character = Right(ConsoleLogLine, Len(ConsoleLogLine) - ConsoleCharacter + 1)
                    Console.Write(Character)
                    ConsoleCharacter = Len(ConsoleLogLine) + 1
                End If
    ...
    Public Class Housekeeping
        Public Shared Sub Maintenance()
            If Not GlobalVariables.Reloading Then
                ...
                For Each TheUnit As UnitInfo In GlobalVariables.KMTUnits
                    LastSeen = TheUnit.LastSeen
                    SecondsAgo = DateDiff(DateInterval.Second, LastSeen, RightNow)
                    If SecondsAgo >= 120 Then
                        Debug(5, "MAINT", "Device " + TheUnit.DeviceIP + " Last Seen " + SecstoWords(SecondsAgo) + " ago Reading Status.")
                        Hardware.ReadUnit(TheUnit.DeviceIP)
                    End If
                Next
                If GlobalVariables.EnableQueue = False Then GlobalVariables.EnableQueue = True
            End If
        End Sub
    End Class
End Module
